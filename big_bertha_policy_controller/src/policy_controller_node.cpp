...
private:
  ...
  void update_commands()
  {
    ...
    double raw_steer = std::clamp(steer_kp_ * err + steer_ki_ * steer_i_, -steer_max_, steer_max_);
    // Rate-limit steering command changes to prevent sudden hip bias jerks.
    steer_cmd_ = std::clamp(raw_steer, last_steer_cmd_ - steer_rate_limit_, last_steer_cmd_ + steer_rate_limit_);
    last_steer_cmd_ = steer_cmd_;
  ...
  void control_loop()
  {
    ...
    if (warming) {
      ...
      bool moving;
      {
        ...
        // Reset steering integrator on !moving→moving transition to prevent
        // windup accumulated during idle/gated stance from causing a steering
        // jerk. This is critical with station_keep_=true where steer_i_ drifts
        // during idle as the robot's contact creep integrates heading error.
        if (!prev_moving_ && moving) {
          steer_i_ = 0.0;
        }
        prev_moving_ = moving;
        if (!moving) {
