...
def generate_launch_description():
    ...
    # over and flip the servo buck-converter switch, so that person is standing
    # next to the robot when the first scan lands, gets mapped as an obstacle
    # overlapping the footprint, and Nav2 then reports a collision before it
    # will plan anything. Waiting does not clear it, which is exactly what was
    # observed. Delaying the first scan past the power-up ritual is the fix;
    # scripts/reset_map.sh recovers a map that already caught someone.
    slam_node = Node(
