...
int main(int argc, char ** argv)
{
  ...
  auto sub = node->create_subscription<sensor_msgs::msg::Image>(
    "/fps_test_cpp", qos, [&](const sensor_msgs::msg::Image::SharedPtr msg) {
  ...
  double p99 =
    latencies_us.empty() ? 0.0 : latencies_us[static_cast<size_t>(latencies_us.size() * 0.99)];
  double max_v = latencies_us.empty() ? 0.0 : latencies_us.back();

  std::printf("=== %s (n=%" PRIu64 ", elapsed=%.3fs) ===\n", label.c_str(), count, elapsed);
  std::printf(
